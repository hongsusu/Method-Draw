...
var addSvgElementFromJson = this.addSvgElementFromJson = function(data) {
  ...
  if (!shape) {
    ...
    if (current_layer) {
      ...
      //   console.log('图层',(current_group || current_layer));
      //   (current_group || current_layer).insertBefore(shape,childNodes[0])
      //   // insertBefore(newItem,list.childNodes[0]);
      // // var m12= $("#svgedit").contents().find("#svgcontent>g[style='pointer-events:all']");
      // // m12.prepend(shape)
      // // console.log('移动',m12)
      // }else{
    (current_group || current_layer).appendChild(shape);
      ...
      // }
      // if(data.element=='image'){
      //  //console.log('ftyuiop',svgedit.utilities.findDefs("g[style='pointer-events:all']"))
      //  debugger
      //   //let curcon=svgedit.utilities.getElem('svgcontent')
      //   // (shape).insertBefore(current_group || current_layer);
      //   //(curcon).appendChild(shape);
      //   debugger
      //  // (current_group || current_layer).prependChild(shape)
      // }else {
        // (current_group || current_layer).appendChild(shape);
        // debugger
      // }
      
     // (shape).prependTo(current_group || current_layer);
    }
  }
  if(data.curStyles) {
...
svgedit.select.init(curConfig, {
  createSVGElement: function(jsonMap) { return canvas.addSvgElementFromJson(jsonMap);console.log('asda进来了') },
...
var getMouseTarget = this.getMouseTarget = function(evt) {
  ...
  console.log('mouse_target',mouse_target);
...
(function() {
  ...
  var mouseDown = function(evt)
  {
    ...
    console.log('svgcontent.querySelector("g")',svgcontent,svgcontent.querySelector("g"))
    debugger
